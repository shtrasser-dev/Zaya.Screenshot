namespace Zaya.Screenshot.Impl.Windows.Constants;

internal static class LocalizationConstants
{
    public static class Engine
    {
        public const string Name = "Cap_EngineName";
        public const string Desc = "Cap_EngineDesc";
    }

    public static class Exceptions
    {
        public const string NotSupported = "Cap_Err_NotSupported";
        public const string ItemCreateFailed = "Cap_Err_ItemCreateFailed";
        public const string WindowItemCreateFailed = "Cap_Err_WindowItemCreateFailed";
        public const string RegionNotSupported = "Cap_Err_RegionNotSupported";
        public const string DisplayIndexOutOfRange = "Cap_Err_DisplayIndexOutOfRange";
        public const string FrameTimeout = "Cap_Err_FrameTimeout";
        public const string CropInvalid = "Cap_Err_CropInvalid";
        public const string CropExceedsBounds = "Cap_Err_CropExceedsBounds";
        public const string PixelFormatNotSupported = "Cap_Err_PixelFormatNotSupported";
        public const string DeviceFailed = "Cap_Err_DeviceFailed";
    }
}
